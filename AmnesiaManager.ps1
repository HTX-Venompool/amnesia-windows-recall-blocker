...
write-host "`n";
write "`$host.ui.RawUI.WindowTitle=`"Windows Recall Wiper Tool`"`nwhile (`$true) {`nif (get-childitem `"`$(`$Env:localappdata)\*\UKP\{*}`" -directory) {`$localappukp = get-childitem `"`$(`$Env:localappdata)\*\UKP\{*}`" -directory;};`nif (get-childitem `"`$(`$Env:appdata)\*\UKP\{*}`" -directory) {`$roamappukp = get-childitem `"`$(`$Env:appdata)\*\UKP\{*}`" -directory;};`n`$localappukp | ForEach-Object {`nif (test-path -path `"`$(`$_)\ImageStore`") {`n`$imagefiles = get-childitem `"`$(`$_)\ImageStore`" -Recurse -File; ForEach (`$imagefile in `$imagefiles.FullName) {write `"DATA EXPUNGED`" | out-file -FilePath `"`$(`$imagefile)`" -Force; del `"`$(`$imagefile)`" -Force | out-null;}; `$imagefiles=`$null; `$imagefile=`$null;`n};`nif (test-path -path `"`$(`$_)\ukg.db`") {write `"DATA EXPUNGED`" | out-file -FilePath `"`$(`$_)\ukg.db`" -Force; del `"`$(`$_)\ukg.db`" -Recurse -Force | out-null;};`nif (test-path -path `"`$(`$_)\ukg.db-shm`") {write `"DATA EXPUNGED`" | out-file -FilePath `"`$(`$_)\ukg.db-shm`" -Force; del `"`$(`$_)\ukg.db-shm`" -Recurse -Force | out-null;};`nif (test-path -path `"`$(`$_)\ukg.db-wal`") {write `"DATA EXPUNGED`" | out-file -FilePath `"`$(`$_)\ukg.db-wal`" -Force; del `"`$(`$_)\ukg.db-wal`" -Recurse -Force | out-null;};`nif (test-path -path `"`$(`$_)\SemanticImageStore.sidb`") {write `"DATA EXPUNGED`" | out-file -FilePath `"`$(`$_)\SemanticImageStore.sidb`" -Force; del `"`$(`$_)\SemanticImageStore.sidb`" -Recurse -Force | out-null;};`nif (test-path -path `"`$(`$_)\SemanticImageStore.sidb.lock`") {write `"DATA EXPUNGED`" | out-file -FilePath `"`$(`$_)\SemanticImageStore.sidb.lock`" -Force; del `"`$(`$_)\SemanticImageStore.sidb.lock`" -Recurse -Force | out-null;};`nif (test-path -path `"`$(`$_)\SemanticImageStore.sidb-shm`") {write `"DATA EXPUNGED`" | out-file -FilePath `"`$(`$_)\SemanticImageStore.sidb-shm`" -Force; del `"`$(`$_)\SemanticImageStore.sidb-shm`" -Recurse -Force | out-null;};`nif (test-path -path `"`$(`$_)\SemanticImageStore.sidb-wal`") {write `"DATA EXPUNGED`" | out-file -FilePath `"`$(`$_)\SemanticImageStore.sidb-wal`" -Force; del `"`$(`$_)\SemanticImageStore.sidb-wal`" -Recurse -Force | out-null;};`nif (test-path -path `"`$(`$_)\SemanticTextStore.sidb`") {write `"DATA EXPUNGED`" | out-file -FilePath `"`$(`$_)\SemanticTextStore.sidb`" -Force; del `"`$(`$_)\SemanticTextStore.sidb`" -Recurse -Force | out-null;};`nif (test-path -path `"`$(`$_)\SemanticTextStore.sidb.lock`") {write `"DATA EXPUNGED`" | out-file -FilePath `"`$(`$_)\SemanticTextStore.sidb.lock`" -Force; del `"`$(`$_)\SemanticTextStore.sidb.lock`" -Recurse -Force | out-null;};`nif (test-path -path `"`$(`$_)\SemanticTextStore.sidb-shm`") {write `"DATA EXPUNGED`" | out-file -FilePath `"`$(`$_)\SemanticTextStore.sidb-shm`" -Force; del `"`$(`$_)\SemanticTextStore.sidb-shm`" -Recurse -Force | out-null;};`nif (test-path -path `"`$(`$_)\SemanticTextStore.sidb-wal`") {write `"DATA EXPUNGED`" | out-file -FilePath `"`$(`$_)\SemanticTextStore.sidb-wal`" -Force; del `"`$(`$_)\SemanticTextStore.sidb-wal`" -Recurse -Force | out-null;};`n};`n`$roamappukp | ForEach-Object {`nif (test-path -path `"`$(`$_)\ImageStore`") {`n`$imagefiles = get-childitem `"`$(`$_)\ImageStore`" -Recurse -File; ForEach (`$imagefile in `$imagefiles.FullName) {write `"DATA EXPUNGED`" | out-file -FilePath `"`$(`$imagefile)`" -Force; del `"`$(`$imagefile)`" -Force | out-null;}; `$imagefiles=`$null; `$imagefile=`$null;`n};`nif (test-path -path `"`$(`$_)\ukg.db`") {write `"DATA EXPUNGED`" | out-file -FilePath `"`$(`$_)\ukg.db`" -Force; del `"`$(`$_)\ukg.db`" -Recurse -Force | out-null;};`nif (test-path -path `"`$(`$_)\ukg.db-shm`") {write `"DATA EXPUNGED`" | out-file -FilePath `"`$(`$_)\ukg.db-shm`" -Force; del `"`$(`$_)\ukg.db-shm`" -Recurse -Force | out-null;};`nif (test-path -path `"`$(`$_)\ukg.db-wal`") {write `"DATA EXPUNGED`" | out-file -FilePath `"`$(`$_)\ukg.db-wal`" -Force; del `"`$(`$_)\ukg.db-wal`" -Recurse -Force | out-null;};`nif (test-path -path `"`$(`$_)\SemanticImageStore.sidb`") {write `"DATA EXPUNGED`" | out-file -FilePath `"`$(`$_)\SemanticImageStore.sidb`" -Force; del `"`$(`$_)\SemanticImageStore.sidb`" -Recurse -Force | out-null;};`nif (test-path -path `"`$(`$_)\SemanticImageStore.sidb.lock`") {write `"DATA EXPUNGED`" | out-file -FilePath `"`$(`$_)\SemanticImageStore.sidb.lock`" -Force; del `"`$(`$_)\SemanticImageStore.sidb.lock`" -Recurse -Force | out-null;};`nif (test-path -path `"`$(`$_)\SemanticImageStore.sidb-shm`") {write `"DATA EXPUNGED`" | out-file -FilePath `"`$(`$_)\SemanticImageStore.sidb-shm`" -Force; del `"`$(`$_)\SemanticImageStore.sidb-shm`" -Recurse -Force | out-null;};`nif (test-path -path `"`$(`$_)\SemanticImageStore.sidb-wal`") {write `"DATA EXPUNGED`" | out-file -FilePath `"`$(`$_)\SemanticImageStore.sidb-wal`" -Force; del `"`$(`$_)\SemanticImageStore.sidb-wal`" -Recurse -Force | out-null;};`nif (test-path -path `"`$(`$_)\SemanticTextStore.sidb`") {write `"DATA EXPUNGED`" | out-file -FilePath `"`$(`$_)\SemanticTextStore.sidb`" -Force; del `"`$(`$_)\SemanticTextStore.sidb`" -Recurse -Force | out-null;};`nif (test-path -path `"`$(`$_)\SemanticTextStore.sidb.lock`") {write `"DATA EXPUNGED`" | out-file -FilePath `"`$(`$_)\SemanticTextStore.sidb.lock`" -Force; del `"`$(`$_)\SemanticTextStore.sidb.lock`" -Recurse -Force | out-null;};`nif (test-path -path `"`$(`$_)\SemanticTextStore.sidb-shm`") {write `"DATA EXPUNGED`" | out-file -FilePath `"`$(`$_)\SemanticTextStore.sidb-shm`" -Force; del `"`$(`$_)\SemanticTextStore.sidb-shm`" -Recurse -Force | out-null;};`nif (test-path -path `"`$(`$_)\SemanticTextStore.sidb-wal`") {write `"DATA EXPUNGED`" | out-file -FilePath `"`$(`$_)\SemanticTextStore.sidb-wal`" -Force; del `"`$(`$_)\SemanticTextStore.sidb-wal`" -Recurse -Force | out-null;};`n};`nSleep -Seconds 1;`n}" | out-file -FilePath "$($PSScriptRoot)\Amnesia.ps1" -Encoding utf8 -Force;
